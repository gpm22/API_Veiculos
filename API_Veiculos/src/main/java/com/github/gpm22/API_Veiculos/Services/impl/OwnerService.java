...
public class OwnerService implements IOwnerService {
    ...
    private void validateOwnerName(String ownerName) throws IllegalArgumentException {
        if (ownerName == null || ownerName.isBlank())
            throw new IllegalArgumentException(
                    "Nome: " + ownerName + " é inválido! Não pode ser apenas espaços em branco.");
    }
    ...
    public Owner getOwnerByCpfOrEmail(String cpfOrEmail) {
        ...
        if (cpfOrEmail.contains("@")) {
            owner = ownerRepository.findByEmail(cpfOrEmail);
        } else {
            String formattedCPF = CPFValidator.formatCPF(cpfOrEmail);
            owner = ownerRepository.findByCpf(formattedCPF);
        }

        if (owner == null)
            throw new IllegalArgumentException(
                    "Não existe usuário com o " + (cpfOrEmail.contains("@") ? "email" : "cpf") + ": " + cpfOrEmail);
